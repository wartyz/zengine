use crate::gl_utilities::gl_buffer::{GLBuffer, AttributeInfo};
...
impl<'a> Sprite<'a> {
    pub fn new(name: &str, shader: &'a Shader, material: Material<'a>,
               width: Option<f32>, height: Option<f32>) -> Sprite<'a> {
        Sprite {
            ...
            u_color_position: shader.get_uniform_location("u_tint"),   // uniform position
    ...
    pub fn draw(&self, model: &Matrix4x4) {
        let u_color_position = self.shader.get_uniform_location("u_tint");   // uniform
        // position
